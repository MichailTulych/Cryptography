...
def key_schedule(key):
    ...
    for k in range(3 * max(len(L), len(S))):
        ...
        B = L[j] = left_rotate(L[j] + A + B, (A + B) % (A.bit_length() + B.bit_length()))
        i = (i + 1) % len(S)
        j = (j + 1) % len(L)
    return S
...
def encrypt(plaintext, S):
    ...
    for i in range(1, ROUNDS + 1):
        A = left_rotate(A ^ B, B % len(bin(A))) + S[2 * i]
        B = left_rotate(B ^ A, A % len(bin(B))) + S[2 * i + 1]
...
def decrypt(ciphertext, S):
    ...
    for i in range(ROUNDS, 0, -1):
        B = right_rotate(B - S[2 * i + 1], A % len(bin(A))) ^ A
        A = right_rotate(A - S[2 * i], B % len(bin(B))) ^ B
    B = B - S[1]
    A = A - S[0]
    return struct.pack('<2I', A, B)
